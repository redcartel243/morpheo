...
2. The Map component requires these properties:
   - center: MUST be an object with {lat: number, lng: number}
   - zoom: number (between 1-20)
   - markers: array of objects with {position: {lat: number, lng: number}, title: string}
   - interactive: boolean
...
IMPORTANT: DO NOT use array format [lat, lng] for coordinates - always use object format {lat: number, lng: number}
